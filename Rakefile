...
def run_tests(scheme, sdk, destination)
  ...
  sh("xcodebuild -workspace PubNub.xcworkspace -scheme '#{scheme}' -sdk '#{sdk}' #{sim_destination} -configuration 'Release' clean test | xcpretty -c ; exit ${PIPESTATUS[0]}") rescue nil
end
