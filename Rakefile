...
def run_tests(scheme, sdk, destination, reports)
  ...
  sh("xcodebuild -workspace PubNub.xcworkspace -scheme '#{scheme}' -sdk '#{sdk}' #{sim_destination} -configuration 'Debug' clean test | " + xcpretty(reports, 'reports/report.html') + "; exit ${PIPESTATUS[0]}") rescue nil
end

def xcpretty(reports, output_destination)
  if reports == true
    xcpretty_command = "xcpretty -c -r html -o #{output_destination}"
  else
