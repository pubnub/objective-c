...
- (void)testJoinEvent {
    ...
    self.otherClientPresenceEventAssertions = ^void (PubNub *client, PNPresenceEventResult *event) {
        ...
        XCTAssertEqualObjects(event.data.channel, [self otherClientChannelName]);
        XCTAssertEqualObjects(event.data.subscription, [self channelGroupName], @"Subscribed channel are not equal.");
        XCTAssertNil(event.data.presence.channel);
        XCTAssertNil(event.data.presence.subscription);
        XCTAssertEqualObjects(event.data.timetoken, @14508283810178448, @"Timetoken is not the same.");
        NSLog(@"------------------------");
...
- (void)testStateChangeEvent {
    ...
    self.otherClientPresenceEventAssertions = ^void (PubNub *client, PNPresenceEventResult *event) {
        ...
        XCTAssertEqualObjects(event.data.timetoken, @14508286718288561, @"Timetoken is not the same.");
        NSLog(@"------------------------");
        [self.presenceEventExpectation fulfill];
    };
    self.presenceEventExpectation = [self expectationWithDescription:@"presenceEvent"];
