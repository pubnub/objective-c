...
class PNPublishTests: PNBasicClientTestCase {
    ...
    func testSimplePublish() {
        self.performVerifiedPublish("test", onChannel: publishTestsChannelName) { (status) -> Void in
            ...
            XCTAssertEqual(status.data.timetoken, 14355311066264140)
        }
    }
    ...
    func testPublishDictionary() {
        self.performVerifiedPublish(["test" : "test"], onChannel: publishTestsChannelName) { (status) -> Void in
            ...
            XCTAssertFalse(status.error)
            XCTAssertEqual(status.data.information, "Sent")
            XCTAssertEqual(status.data.timetoken, 14355311062532489)
        }
    }
    
    func testPublishNestedDictionary() {
        self.performVerifiedPublish(["test" : ["test": "test"]], onChannel: publishTestsChannelName) { (status) -> Void in
            XCTAssertNotNil(status)
            XCTAssertEqual(status.category, PNStatusCategory.PNAcknowledgmentCategory)
            XCTAssertEqual(status.operation, PNOperationType.PublishOperation)
            XCTAssertEqual(status.statusCode, 200)
            XCTAssertFalse(status.error)
            XCTAssertEqual(status.data.information, "Sent")
            XCTAssertEqual(status.data.timetoken, 14355311063449364)
        }
    }
    
    func testPublishNumber() {
        self.performVerifiedPublish(700, onChannel: publishTestsChannelName) { (status) -> Void in
            ...
            XCTAssertFalse(status.error)
            XCTAssertEqual(status.data.information, "Sent")
            XCTAssertEqual(status.data.timetoken, 14355311064365833)
        }
    }
    
    func testPublishArray() {
        self.performVerifiedPublish(["1", "2", "3", "4"], onChannel: publishTestsChannelName) { (status) -> Void in
            ...
            XCTAssertFalse(status.error)
            XCTAssertEqual(status.data.information, "Sent")
            XCTAssertEqual(status.data.timetoken, 14355311060753687)
        }
    }
    
    func testPublishComplexArray() {
        self.performVerifiedPublish(["1", ["1": ["1": "2"]], ["1", "2", 2], 567], onChannel: publishTestsChannelName) { (status) -> Void in
            ...
            XCTAssertFalse(status.error)
            XCTAssertEqual(status.data.information, "Sent")
            XCTAssertEqual(status.data.timetoken, 14355311061679200)
        }
    }
    
    func testPublish10kCharactersString() {
        ...
        self.performVerifiedPublish(longString, onChannel: publishTestsChannelName) { (status) -> Void in
            ...
            XCTAssertFalse(status.error)
            XCTAssertEqual(status.data.information, "Sent")
            XCTAssertEqual(status.data.timetoken, 14355311058624908)
        }
    }
    
    func testPublishStringWithSpecialSymbols() {
        
        let stringWithSpecialSymbols = "!@#$%^&*()_+|"
        
        self.performVerifiedPublish(stringWithSpecialSymbols, onChannel: publishTestsChannelName) { (status) -> Void in
            ...
            XCTAssertFalse(status.error)
            XCTAssertEqual(status.data.information, "Sent")
            XCTAssertEqual(status.data.timetoken, 14355311065294161)
        }
    }
    
    func testPublishWithMobilePushPayload() {
    
        let message = ["Hello" : "world"];
        let mobilePayload = ["apns":["alert":"Hello from PubNub"]];
        
        let networkExpectation = self.expectationWithDescription("network")
        client.publish(message, toChannel: publishTestsChannelName, mobilePushPayload: mobilePayload, withCompletion: { (status) -> Void in
            ...
            XCTAssertFalse(status.error)
            XCTAssertEqual(status.data.information, "Sent")
            XCTAssertEqual(status.data.timetoken, 14371442800390842)
    ...
    func testPublishWithMobilePushPayloadCompressed() {
        ...
        client.publish(message, toChannel: publishTestsChannelName, mobilePushPayload: mobilePayload, compressed: true, withCompletion: { (status) -> Void in
            ...
            XCTAssertFalse(status.error)
            XCTAssertEqual(status.data.information, "Sent")
            XCTAssertEqual(status.data.timetoken, 14371463879074307)
    ...
    func testPublishWithMobilePushPayloadStoredInHistory() {
        ...
        client.publish(message, toChannel: publishTestsChannelName, mobilePushPayload: mobilePayload, storeInHistory: false, withCompletion: { (status) -> Void in
            ...
            XCTAssertFalse(status.error)
            XCTAssertEqual(status.data.information, "Sent")
            XCTAssertEqual(status.data.timetoken, 14371496138044862)
    ...
    func testPublishWithMobilePushPayloadStoredInHistoryCompressed() {
        ...
        client.publish(message, toChannel: publishTestsChannelName, mobilePushPayload: mobilePayload, storeInHistory: false, compressed: true, withCompletion: { (status) -> Void in
            ...
            XCTAssertFalse(status.error)
            XCTAssertEqual(status.data.information, "Sent")
            XCTAssertEqual(status.data.timetoken, 14371507370857331)
