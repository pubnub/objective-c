 ...
 * @since 4.13.0
 */
@property (nonatomic, readonly, strong) PNFetchSpacesAPICallBuilder * (^sort)(NSArray<NSString*> *sort);

/**
 * @brief Expression to filter out results basing on specified criteria.
 *
 * @param filter Spaces filter expression.
 *
 * @return API call configuration builder.
 *
 * @since 4.13.0
 */
@property (nonatomic, readonly, strong) PNFetchSpacesAPICallBuilder * (^filter)(NSString *filter);
