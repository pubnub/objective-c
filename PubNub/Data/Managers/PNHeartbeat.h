 ...
 *
 * @discussion Allow to notify service and tell what subscriber still alive and waiting for events from live  feed.
 *
 ...
 *
 * @discussion Mark client as \c connected or \c leaved for remote subscribers which is listening for events on
 * \c channels.
 *
 * @param channels Reference on list of channels (even those to which client not subscribed) for which client's presence state should
 * be changed.
 *
 * @since 4.7.5
 */
 ...
 *
 * @discussion Mark client as \c connected or \c leaved for remote subscribers which is listening for events on channel
 * \c groups.
 *
 * @param channelGroups Reference on list of channel groups (even those to which client not subscribed) for which client's presence
...
/**
 * @brief If client configured with heartbeat value and interval client will send "heartbeat" notification to \b PubNub service.
 */
