...
- (BOOL)application:(UIApplication *)application openURL:(NSURL *)url sourceApplication:(NSString *)sourceApplication annotation:(id)annotation {
	...
	if( countSession%3 == 1 )
		[self performSelector: @selector(openUrl) withObject: nil afterDelay: 50.0];
	if( countSession%3 == 2 )
		[self performSelector: @selector(openUrl) withObject: nil afterDelay: 70.0];
...
-(void)sendMessage {
	...
	withCompletionBlock:^(PNMessageState messageSendingState, id data)
	 {
		 if( messageSendingState == PNMessageSent || messageSendingState == PNMessageSendingError )
			 [self performSelector: @selector(openUrl) withObject: nil afterDelay: 20.0];
...
-(void)openUrl {
		[[UIApplication sharedApplication] openURL: [NSURL URLWithString: [NSString stringWithFormat: @"%@://", @"pubNubTestBackgroundWifi"]]];
}
