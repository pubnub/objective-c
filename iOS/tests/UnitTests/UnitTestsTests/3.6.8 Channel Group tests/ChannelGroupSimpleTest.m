...
- (void)testChannelGroupFunction {
    ...
    [self checkOn:false isNamespace:_testNamespace];
    
    // Add channels, also are added group and namespace
    [self addChannels:_testChannels toGroup:_testGroup];
    [self checkOn:true isChannels:_testChannels inGroup:_testGroup];
    [self checkOn:true isGroup:_testGroup inNamespace:_testNamespace];
    [self checkOn:true isNamespace:_testNamespace];

    // Subscribe on group and unsubscribe from group
    [self subscribeOnGroups:_testGroups];
    [self checkOn:true isClientSubscribedOn:_testGroup];
    [self unsubscribeFromGroups:_testGroups];
    [self checkOn:false isClientSubscribedOn:_testGroup];
    ...
    [self checkOn:false isChannels:_testChannels inGroup:_testGroup];
    [self checkOn:false isGroup:_testGroup inNamespace:_testNamespace];
    [self checkOn:true isNamespace:_testNamespace];
...
- (void)testChannelGroupDuplicationMethods {
    ...
    [self subscribeOnGroups:_testGroups];
//    [self subscribeOnGroups:_testGroups];  // error !!!
    [self checkOn:true isClientSubscribedOn:_testGroup];
    [self checkOn:true isClientSubscribedOn:_testGroup];
...
//    [self didFailRemoveGroup:_testGroup]; // error !!!
...
- (void)checkOn:(bool)positive isChannels:(NSArray *)channels inGroup:(PNChannelGroup *)group {
...
- (void)checkOn:(BOOL)positive isGroup:(PNChannelGroup *)group inNamespace:(NSString *)space  {
...
- (void)checkOn:(BOOL)positive isNamespace:(NSString *)name {
...
- (void)subscribeOnGroups:(NSArray *)groups {
...
- (void)unsubscribeFromGroups:(NSArray *)groups {
    ...
    [_testClient.observationCenter addClientChannelUnsubscriptionObserver:self withCallbackBlock:^(NSArray *channels, PNError *error) {
    ...
    // Execution of the method using block
    [_testClient unsubscribeFrom:groups withCompletionHandlingBlock:^(NSArray *channels, PNError *error) {
...
-(void)checkOn:(BOOL)positive isClientSubscribedOn:(PNChannelGroup *)group {
    
    if (positive & ![_testClient isSubscribedOn:_testGroup]) {
        
        XCTFail(@"The checking showed an error");
        @throw _stopTestException;
        
    } else if (!positive & [_testClient isSubscribedOn:_testGroup]) {
...
//    [self checkIsChannels:_testChannels inGroup:_testGroup];
