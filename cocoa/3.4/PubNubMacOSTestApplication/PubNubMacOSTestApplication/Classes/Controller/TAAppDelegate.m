...
- (void)subscribeOnChannels {
    ...
    TAAppDelegate *weakSelf = self;
    [PubNub subscribeOnChannel:[PNChannel channelWithName:kTAChannelName]
   withCompletionHandlingBlock:^(PNSubscriptionProcessState state, NSArray *channels, PNError *subscriptionError) {
       
       NSString *logMessage = [NSString stringWithFormat:@"Subscribed on channels: %@", [channels valueForKey:@"name"]];
       if (state == PNSubscriptionProcessNotSubscribedState) {
           
           logMessage = [NSString stringWithFormat:@"Failed to subscribe on: %@", [channels valueForKey:@"name"]];
       }
       
       
       PNLog(PNLogGeneralLevel, weakSelf, logMessage);
       
       
       if (state == PNSubscriptionProcessSubscribedState) {
           
           [weakSelf publishMessage];
           
           double delayInSeconds = 2.0;
           dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds * NSEC_PER_SEC));
           dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
               
               [weakSelf fetchHereNow];
           });
       }
   }];
}
...
- (void)publishMessage {
    ...
    TAAppDelegate *weakSelf = self;
    [PubNub sendMessage:@{@"title":@"JSON dictionary encapsulated message",@"body":@"This is example how we can send dictionary object."}
              toChannel:[PNChannel channelWithName:kTAChannelName]
...
- (void)fetchHistory {
    ...
    TAAppDelegate *weakSelf = self;
    [PubNub requestHistoryForChannel:[PNChannel channelWithName:kTAChannelName]
                                from:nil
                                  to:nil
                               limit:100
                 withCompletionBlock:^(NSArray *messages,
                     ...
                     [messages enumerateObjectsUsingBlock:^(PNMessage *message,
                         ...
                         PNLog(PNLogGeneralLevel, weakSelf, [NSString stringWithFormat:@"> %@\n", message.message]);
                     }];
                 }];
}
...
- (void)fetchHereNow {
    ...
    TAAppDelegate *weakSelf = self;
    [PubNub requestParticipantsListForChannel:[PNChannel channelWithName:kTAChannelName]
                          withCompletionBlock:^(NSArray *participants, PNChannel *channel, PNError *error) {
                              
                              if (error == nil) {
                                  
                                  PNLog(PNLogGeneralLevel, weakSelf, @"Received list of participants on channel (%@):\n%@", participants, channel);
                              }
                              else {
                                  
                                  PNLog(PNLogGeneralLevel, weakSelf, @"Participants list request error: %@", error);
                              }
                          }];
}
